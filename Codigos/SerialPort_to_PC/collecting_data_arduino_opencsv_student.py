...
try:
    while True:
        now = "Obtain date with library datetime that gives the current local date and time"
        # SEND MESSAGE - Actuator (Relay, LED, etc.)
        """if (now.second%10<5):
        ...
        # READ DATA    
        # Check if there is new info from the Arduino and read it (.readline)
        data_bytes = 
